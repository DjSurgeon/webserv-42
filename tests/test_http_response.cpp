...
void test_serialization() {
    ...
    bool check_header2 = (s.find("Content-Type: text/html\r\n") != std::string::npos);
    bool check_body = (s.find("\r\n\r\nHello World") != std::string::npos);

    print_result("test_serialization", check_status && check_header1 && check_header2 && check_body);
}
...
void test_edge_cases() {
    ...
    // Massive Body (approx 1MB for unit test speed, 10MB in stress script if needed)
    ...
    bool header_flood_pass = (s_headers.find("X-Header-999:") != std::string::npos);

    print_result("test_edge_cases", huge_body_pass && header_flood_pass);
}

void test_stress() {
    std::cout << "[Test] Stress test: 50,000 response generations..." << std::endl;
