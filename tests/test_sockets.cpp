...
void test_privileged_port() {
  std::cout << "[Test] Binding to privileged port 80 (root required)..."
            << std::endl;
  ListeningSocket ls;
  try {
    ls.init(80);
    print_result("test_privileged_port",
                 false);  // Should not reach here if not root
...
void test_double_bind() {
  std::cout << "[Test] Double bind on the same ListeningSocket instance..."
            << std::endl;
  ListeningSocket ls;
  try {
    ls.init(8081);
...
void test_invalid_port() {
  std::cout << "[Test] Binding to an out-of-range port (999999)..."
            << std::endl;
  ListeningSocket ls;
  try {
...
void test_closed_fd() {
  std::cout << "[Test] Initializing ClientSocket with non-existent FD 9999..."
            << std::endl;
  try {
    ClientSocket cs(9999);
    print_result("test_closed_fd", false);
  } catch (const std::exception& e) {
    std::cout << "Caught expected exception (fcntl should fail): " << e.what()
              << std::endl;
...
void test_file_fd() {
  std::cout << "[Test] Initializing ClientSocket with a regular file FD..."
            << std::endl;
...
void test_explicit_constructor() {
  std::cout << "[Test] Initializing ListeningSocket with explicit constructor "
               "on port 8083..."
            << std::endl;
  try {
    ...
    try {
      ...
      std::cout << "Caught expected exception on double init: " << e.what()
                << std::endl;
...
void test_client_socket_buffers() {
  std::cout << "[Test] ClientSocket Buffer manipulation (Option C)..."
            << std::endl;
