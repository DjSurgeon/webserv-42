...
/**
 * @brief Simula el comportamiento del EventLoop real conectando el buffer y el parser.
 * Consume los caracteres del ClientSocket uno a uno y limpia el buffer al terminar.
 */
static e_parser_state run_integration_pipeline(ClientSocket& client, RequestParser& parser, const std::string& raw_network_data) {
    ...
    for (size_t i = 0; i < buffer.length(); ++i) {
        ...
        if (current_state == STATE_ERROR || current_state == STATE_HEADER_KEY) {
            // Detener el procesamiento si ocurre un error o si se completa la Request-Line
            break;
        }
    }

    // 4. El parser le indica al cliente cuántos bytes del búfer puede consumir (borrar)
...
void test_fragmented_network_stream() {
    ...
    e_parser_state final_state = run_integration_pipeline(client, parser, "TP/1.1\r\n");
...
void test_attack_double_space() {
    ...
    print_test_result("Attack Mitigation: Double Space", final_state == STATE_ERROR);
}
...
void test_attack_wrong_version() {
    ...
    print_test_result("Attack Mitigation: HTTP/1.2 Rejection", final_state == STATE_ERROR);
}

int main() {
    std::cout << "🧪 RUNNING WEBSERV INTEGRATION TEST BATTERY (CLIENT_SOCKET + PARSER)\n";
    std::cout << "---------------------------------------------------------------\n";
