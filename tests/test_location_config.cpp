...
void test_setters_and_getters() {
    ...
    if (methods.size() != 2 || methods[0] != "GET" || methods[1] != "POST") pass = false;
...
void test_canonical_form() {
    std::cout << "[Test] Verifying deep copy and assignment (including Context)..." << std::endl;
    LocationConfig loc1;
    loc1.set_path("/old");
    ...
    bool copy_pass = (loc2.get_path() == "/old" && 
                      loc2.get_allowed_methods().size() == 1 &&
                      loc2.get_root() == "/old/root");

    // Assignment
    LocationConfig loc3;
    loc3 = loc1;
    bool assign_pass = (loc3.get_path() == "/old" && 
                        loc3.get_allowed_methods().size() == 1 &&
    ...
    bool deep_pass = (loc2.get_path() == "/old" && loc2.get_allowed_methods().size() == 1);

    print_result("test_canonical_form", copy_pass && assign_pass && self_assign_pass && deep_pass);
}

void test_edge_cases() {
    std::cout << "[Test] Verifying edge cases (empty strings and massive vectors)..." << std::endl;
    LocationConfig loc;

    // Empty path and methods
    loc.set_path("");
    loc.set_allowed_methods(std::vector<std::string>());
    bool empty_pass = (loc.get_path().empty() && loc.get_allowed_methods().empty());
...
void test_stress() {
    std::cout << "[Test] Stress test: 50,000 instantiations/copies..." << std::endl;
