...
int main(int argc, char** argv) {
    ...
    if (mode == MODE_UNKNOWN) {
        std::cerr << "Usage: " << argv[0] << " [--mode flood|slowloris|garbage|drop] [--count N] [--port P]" << std::endl;
        return 1;
    }

    std::cout << "Starting stress client: mode=" << mode << ", count=" << count << ", port=" << port << std::endl;
        ...
        for (size_t i = 0; i < pfds.size(); ++i) {
            ...
            if (pfds[i].revents & POLLOUT) {
                if (mode == MODE_SLOWLORIS) {
                    ...
                    send(conns[i].fd, conns[i].to_send.c_str(), conns[i].to_send.length(), 0);
                    close(conns[i].fd);
                    conns[i].fd = -1;
                    conns[i].completed = true;
                    active--;
                    ...
                    if (s > 0) {
                        ...
                        if (conns[i].sent >= conns[i].to_send.length()) {
                            ...
                            if (mode == MODE_GARBAGE) {
                                // For garbage, we don't necessarily expect a full response before closing, 
                                // but we'll wait for POLLIN for a bit.
                            }
                        }
                    }
                }
            ...
            if (mode == MODE_SLOWLORIS && time(NULL) - conns[i].last_action > 5 && conns[i].sent > 10) {
                close(conns[i].fd);
                conns[i].fd = -1;
                conns[i].completed = true;
                active--;
