...
void test_double_crlf_transition() {
    ...
    bool check1 = (state1 == STATE_HEADER_KEY); // expecting newline flag set, state unchanged yet
...
void test_empty_header_key_error() {
    std::cout << "[Test] Error on colon separator at start of line..." << std::endl;
    RequestParser parser;
...
void test_invalid_header_key_char_error() {
    ...
    // A space character ' ' is illegal inside header keys (RFC 7230 3.2.4: field-name = token, no space allowed)
